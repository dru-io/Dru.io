...
/**
 * Migration for comments related to node type post.
 *
 * This migration get all comments from database, but on preprate state skips
 * unnecessary comments. There is no other simple way, because we cant use joins
 * in initial query.
 *
 * @MigrateSource(
 *   id = "druio_node_post_comments"
 * )
 */
class NodePostComments extends SqlBase {
  ...
  public function prepareRow(Row $row) {
    ...
    $is_comment_to_post = $this->select('node', 'n')
      ->fields('n')
